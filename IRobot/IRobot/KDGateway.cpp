...
//---------------------------------------------------------------------------
//登录
BOOL CKDGateway::Login(char p_cUserType, char *p_pszUserID, char *p_pszPassword)
{
	CBlowFish oBlowFish((unsigned char *)m_szWorkKey, strlen(m_szWorkKey));
	oBlowFish.Encrypt((unsigned char*)p_pszPassword, (unsigned char*)p_pszPassword, strlen(p_pszPassword));

	//登录时 包头的用户代码需使用资金帐号
	char szTemp[512];
	sprintf_s(szTemp, sizeof(szTemp), "409101|%c|%s|%s|||||||00247E030C03|0001067a|S1XEJ9DZ500210",
		p_cUserType, p_pszUserID, p_pszPassword);
	...
	// 对柜台返回的值进行解析
	ResultStrToTable(m_pReturnData);
...
int CKDGateway::ResultStrToTable(const char *pReturnData)
{
	int nRecNo = GetRecNum();
	MID_LOGIN *m_pLogin_info = new MID_LOGIN[nRecNo];
	memset(m_pLogin_info, 0x00, sizeof(MID_LOGIN)*nRecNo);
	
	char *p = (char *)pReturnData;
	
	// 获得第二行，即数据开始行
	char *q = GetNextLine(p);
	char *tmp = q;
	
	// 获取每行的数据
	for (DWORD nRow=0; nRow<GetRecNum(); nRow++)
	{
		// 获取每列的长度
		for (DWORD nCol=0; nCol<GetFieldNum(); nCol++)
		{
			while(tmp++)
			{
				if (*tmp == '|')
				{
					*tmp = '\0';					
					...
					{
					case 0:
						strncpy_s(m_pLogin_info[nRow].szUserCode, q, 9);
						break;
					case 1:
						strncpy_s(m_pLogin_info[nRow].szMarket, q, 3);
						break;
					case 2:
						strncpy_s(m_pLogin_info[nRow].szBoard, q, 2);
						break;
					case 3:
						strncpy_s(m_pLogin_info[nRow].szSecuAcc, q, 15);
						break;
					case 4:
						strncpy_s(m_pLogin_info[nRow].szSecuAccName, q, 20);
						break;
					case 5:
						strncpy_s(m_pLogin_info[nRow].szAccount, q, 10);
						break;
					case 6:
						strncpy_s(m_pLogin_info[nRow].szUserName, q, 20);
						break;
					case 7:
						strncpy_s(m_pLogin_info[nRow].szBrh, q, 10);
						break;
					case 8:
						strncpy_s(m_pLogin_info[nRow].szMainFlag, q, 2);
						break;
					case 9:
						strncpy_s(m_pLogin_info[nRow].szSession, q, 20);
						break;
					case 10:
						strncpy_s(m_pLogin_info[nRow].szTaCode, q, 5);
						break;
					case 11:
						strncpy_s(m_pLogin_info[nRow].szCertDays, q, 2);
						break;
					case 12:
						strncpy_s(m_pLogin_info[nRow].szEtokenDays, q, 2);
						break;
					}
