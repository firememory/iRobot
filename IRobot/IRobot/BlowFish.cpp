...
//Returns false if n is multiple of 8
void CBlowFish::Encrypt(const unsigned char* in, unsigned char* out, size_t k, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	unsigned char szinbuf[200],szoutbuf[200],sztrueoutbuf[200];

	unsigned char *pszinbuf, *pszoutbuf;
	pszinbuf = (unsigned char *)&szinbuf[0];
	pszoutbuf = (unsigned char *)&szoutbuf[0];
	...
	{
		SBlock chain(m_oChain);
		for(; n >= 8; n -= 8, pszinbuf += 8)
		{
			BytesToBlock(pszinbuf, work);
			work ^= chain;
			Encrypt(work);
			chain = work;
			BlockToBytes(work, pszoutbuf+=8);
		}
	}
	else if(iMode == CFB) //CFB mode, using the Chain
	{
...
//Returns false if n is multiple of 8
void CBlowFish::Decrypt(const unsigned char* in, unsigned char* out, size_t k, int iMode)
{
	unsigned char szinbuf[200], sztrueinbuf[200],szoutbuf[200],sztrueoutbuf[200];
	...
	len =n;
	HexStr2CharStr((const char *)szinbuf,sztrueinbuf,len);

	pszinbuf = (unsigned char *)&sztrueinbuf[0];
	n = len/2;
	//Check the buffer's length - should be > 0 and multiple of 8
	if((n==0)||(n%8!=0))
		throw std::exception("Incorrect buffer length");
	...
	len = strlen((const char *)szoutbuf);
	len --;
	while (szoutbuf[len] ==0x20 && len !=0)
	{
		szoutbuf[len]=0x00;
		len--;
	}
...
void CBlowFish::Char2Hex(const unsigned char ch, char* szHex)
{
...
void CBlowFish::Hex2Char(const char* szHex, unsigned char& rch)
{
	rch = 0;
	for(int i=0; i<2; i++)
	{
...
void CBlowFish::CharStr2HexStr(const unsigned char* pucCharStr, char* pszHexStr, int iSize)
{
	int i;
	char szHex[3];
	pszHexStr[0] = 0;
...
void CBlowFish::HexStr2CharStr(const char* pszHexStr, unsigned char* pucCharStr, int iSize)
{
	int i;
	unsigned char ch;
	for(i=0; i<iSize; i++)
